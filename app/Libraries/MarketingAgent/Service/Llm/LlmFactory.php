...
class LlmFactory {
    public static function create(DatabaseService $db, ?string $overrideProvider = null, ?int $userId = null): LlmProviderInterface {
        ...
        // Let plugins provide a custom LLM provider instance
        $inner = \MarketingAgent\Plugin\HookManager::applyFilters('llm_provider_instance', null, $provider, $settings);
        ...
        if ($inner === null) {
            switch ($provider) {
                ...
                case 'openrouter':
                    ...
                    $siteUrl  = $settings['openrouter_site_url'] ?? '';
                    $siteName = $settings['app_name'] ?? 'Marketing AI Agent';
                    $inner    = new OpenRouterProvider($apiKey, $model, $siteUrl, $siteName);
                    break;

                case 'openai_compatible':
                    $url    = $settings['openai_compat_url'] ?? 'https://api.openai.com/v1';
                    $model  = $settings['openai_compat_model'] ?? 'gpt-4o-mini';
                    $apiKey = $settings['openai_compat_api_key'] ?? '';
                    $inner  = new OpenAICompatibleProvider($url, $model, $apiKey);
                    break;
